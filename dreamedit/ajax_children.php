<?php
    ...
    foreach($rows as $k => $v) {
        ...
        if ($v[page_template]=='jnumber') {
            $elmnts[$count][text] = normJsonStr(str_replace("Мировая экономика и международные отношения", "МЭиМО", $v['j_name'])."-".$v["year"]."-".$v["page_name"]);
            $elmnts[$count][a_attr][title] = $countJnumber;
        }
        else
