...
if ($_SESSION[jour]=='/jour')  /// Сделать для многих журналов
{
...
if ($_SESSION[jour]=='/jour') $main=$_SESSION[lang].'/jour/'.$_SESSION[jour_url];
	 ...
	 foreach($submenu0 as $k=>$submenu)
      {
		...
		if (substr($submenu[page_template],0,8)=='magazine' || $_SESSION[jour]=='/jour')
			$prefjour="/jour/".$_SESSION[jour_url];
		else
            $prefjour="";		
...
//	if(strpos($pg->getPageUrl($row["page_id"]),"?")!==false) $pref="&"; else $pref="?";
        ...
        $link_false='';
        if($row['link_false'])
            $link_false = 'return false; ';
        $pref="";
		if ($_SESSION[jour]=='/jour' && ($row[page_template]=='text' || $row[page_template]=='magazine_page_archive')) 
		$page=$pagej; else $page="";
		if ($_SESSION[lang]!='/en')
        echo	"<li id='nav' ".
        "  class='first-level' ><a class='first-level ".$classname.
        "'  id=\"m\" onmouseout=\"this.id='m';\" onmouseover=\"this.id='over_m';\" onclick=\"".$link_false."document.location='".$prefjour."/index.php?page_id=".$page.
        $row["page_id"]."'\" nowrap=\"nowrap\">".$row["page_name"]."</a>";
        else
       echo	"<li id='nav' ".
        "  class='first-level' ><a class='first-level ".$classname.
        "'  id=\"m\" onmouseout=\"this.id='m';\" onmouseover=\"this.id='over_m';\" onclick=\"".$link_false."document.location='/en".$prefjour."/index.php?page_id=".$page.
...
if($row[page_id]==494 && $_SESSION[lang]!="/en")
	$opt_page=-1;
...
for ($j=0;$j<((count($kk)/$step)+$opt_page);$j++)

        {
		   ...
		   if (!isset($_REQUEST[en]))
			 ...
			 if ($_SESSION[jour]=='/jour' && ($submenu[page_template]=='text' || $submenu[page_template]=='magazine_page_archive')) 
			 $page=$pagej; else $page="";
            ...
            echo '
			   ...
			   if (count($submenu2)>0 && count($submenu2)<$submenu_count) echo "<ul class='topnav-dd-inner-sub'>";
				 ...
				 {
				 if($page.$sub2[page_id]!=1037)
					continue;
				
				if (substr($sub2[page_template],0,8)=='magazine'|| $_SESSION[jour]=='/jour')
					$prefjour="/jour/".$sub2[page_journame];
				else
				$prefjour="";		
				 if ($_SESSION[jour]=='/jour' && ($row[page_template]=='text' || $row[page_template]=='magazine_page_archive')) 
				 $page=$pagej; else $page="";
                ...
                if ($sub2[page_template]!='news_full' && $sub2[page_template]!='1publ' && $sub2[page_template]!='pers_full')
				{
				    ...
				    if($prefjour=="") {
                        ...
                        if ($_SESSION[lang] != '/en')
                            echo '<a href=' . $prefjour . '>' . $sub2[page_name] . '</a>';
                        else
                            echo '<a href=' . $prefjour . '>' . $sub2[page_name_en] . '</a>';
                    }
                }
				else
				{
...
else
{
       ...
       for ($j=0;$j<(count($kk)/$step);$j++)
        ...
        $step_max = count($kk) / $step * 2;

		if($row[page_id]==492)
		   $step_max = $j+1;

		for ($j=$j;$j<$step_max;$j++)
        {
           $submenu=$submenu0[$kk[$j]];
           if (!isset($_REQUEST[en]))
				$submenu2=$pg->getChildsMenu($submenu[page_id],1);
	       ...
	       if (count($submenu2)>0)
            ...
            if (substr($row[page_template],0,8)=='magazine'|| $_SESSION[jour]=='/jour')
					$prefjour="/jour/".$_SESSION[jour_url];
				else
				$prefjour="";	
				
			if($page.$submenu[page_id]==1037)
				continue;
             ...
             echo '<li><a href=/en'.$prefjour.'/index.php?page_id='.$page.$submenu[page_id].'>'.$submenu[page_name_en].'</a>';
              ...
              if ($_SESSION[jour]=='/jour' && ($row[page_template]=='text' || $row[page_template]=='magazine_page_archive')) $page=$pagej; else $page="";
			  
			  
			   if (count($submenu2)>0 && count($submenu2)<16) echo "<ul class='topnav-dd-inner-sub'>";
               {
				 foreach($submenu2 as $sub2)
				 {
         //       print_r($sub2);
				if (substr($sub2[page_template],0,8)=='magazine') $prefjour='/jour/'.$sub2[page_journame];
				else $prefjour="";
					if ($sub2[page_template]!='news_full' && $sub2[page_template]!='1publ' && $sub2[page_template]!='pers_full' && $sub2[page_template]!= 'search_persona')
					{
						if($page.$sub2[page_id]==1037)
							continue;
						if($row[page_id]==492 || $row[page_id]==495)
							echo "<li></li>";
						if($prefjour=="")
						{
						 if ($_SESSION[lang]!='/en')
						...
						{
							if ($_SESSION[lang]!='/en')
							echo '<li><a href='.$prefjour.'>'.@$sub2[page_name].'</a></li>';
						 else	
							echo '<li>	<a href=/en'.$prefjour.'>'.@$sub2[page_name_en].'</a></li>';
						}
					 }
				 }
				}

				if (count($submenu2)>0 && count($submenu2)<16) echo "</ul>";
                ...
                <div id="block-nodeblock-43333" class="clear-block block block-nodeblock ">
                    <div class="content">
                        ...
                        <p>
                            ...
                            <a style="float: right;"
                               href="<? preg_match_all('/<a href=\"(.*)\">(.*)<\/a>/', $row[menu_link], $match);
                               echo $match[1][0]; ?>">
                                <?= @$row[menu_picture]; ?>
                            </a>
                        </p>
