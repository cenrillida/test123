...
foreach ($videos as $video): $counter++;?>
    ...
    <div class="col-lg-3 col-md-6 col-xs-12 pb-3<?php if($counter>2) echo ' d-none d-lg-block';?>">
        <div class="embed-responsive embed-responsive-16by9">
            <iframe class="embed-responsive-item" data-src="//<?=$video['youtube_url']?>?autoplay=true&height=100%25&width=100%25<?php if(!empty($video['time_seconds'])) echo '&start='.$video['time_seconds']. "&t=" . $video['time_seconds'];?>" allowfullscreen></iframe>
            <img class="video-iframe-image" src="<?=$video['photo_stop']?>" alt="">
            <div class="video-iframe-button-div">
                <button class="btn btn-danger video-iframe-button" type="button"><i class="fas fa-video"></i> <?php if($_SESSION[lang]!='/en') echo 'Смотреть'; else echo 'Watch';?></button>
            </div>
        </div>
