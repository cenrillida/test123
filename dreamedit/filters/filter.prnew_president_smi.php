...
$query = "SELECT c.el_id AS id,c.icont_text AS prev_text,rub.icont_text AS rubric,a.icont_text AS title,pe.icont_text AS people,sp.icont_text AS small_picture,d.icont_text AS date,f.icont_text AS full_text,
                 ...
                 LEFT JOIN adm_ilines_content AS dyn ON dyn.el_id=a.el_id AND dyn.icont_var='dynkin_smi'
                 ...
                 ORDER BY d.icont_text DESC LIMIT 0,6
                ";
...
<div class="d-flex justify-content-center president-smi-loader-main">
    <div class="spinner-border president-smi-loader mt-5" role="status" style="display: none">
        <span class="sr-only">Loading...</span>
    </div>
</div>


<div class="col-12 py-3 more-button-president-smi text-center">
    <div class="pr-red-title-a-main"><button class="pr-red-title-a pl-2 pr-2" target="_blank"><i class="fas fa-chevron-down"></i></button> <button class="pr-red-title-a-back president-smi-ajax" target="_blank"><i class="fas fa-chevron-down"></i></button></div>
</div>

<script>
    president_smi_count = 6;
    $( ".president-smi-ajax" ).on( "click", function(event) {
        event.preventDefault();
        jQuery.ajax({
            type: 'GET',
            url: '/index.php?page_id=<?=$_REQUEST[page_id]?>&ajax_mode=1&ajax_president_smi='+president_smi_count,
            success: function (data) {
                president_smi_count += 3;
                $('.president-smi-loader-main').before(data);
            },
            complete: function (data) {
                $('.president-smi-loader').hide();
                $('.more-button-president-smi').show();
            },
            beforeSend: function () {
                $('.president-smi-loader').show();
                $('.more-button-president-smi').hide();
            }
        })
    });
</script>