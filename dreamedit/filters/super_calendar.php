...
$dbuser = 'imemo';//$_GET[dbuser];		// database username
$dbpass = '_3517';//$_GET[dbpass];		// database password
$dbname = 'imemo';//$_GET[dbname];		// database password
//////////////////////////////////////
...
// подключаем клас для работы с календарем
...

{
    ...
    if (substr($ll,0,1)=="0")
       $ll=substr($month,1,1);
	$name = $month_name[$ll];  // Поставить месяц по-русски
    ...
    $refmon="/index.php?page_id=".$page_id."&td1=".sprintf('%04d.%02d.%02d',$year2,$month,'01').
            "&td2=".sprintf('%04d.%02d.%02d', $year2, $month, $days0); //$year2.".".$month.".".$days0;
    $refyear="/index.php?page_id=".$page_id."&td1=".$year2.".01.01".
            "&td2=".$year2.".12.31";
				...
				<td colspan='8' class='calhead'>
					...
					<tr>
						<td>
							...
							)' style='border:none'><img src='/calendar/images/calLeft.gif' title='Предыдущий месяц' /></a>
							<a href='javascript:navigate(\"\",\"\",\"\",\"$dbhost\",\"$dbname\",\"$dbuser\",\"$dbpass\",\"$ilines_spisok\",\"$page_id\")'
									style='border:none'><img src='/calendar/images/calCenter.gif' title='Текущая дата' /></a>
							<a href='javascript:navigate($next,$nexty,\"\",\"$dbhost\",\"$dbname\",\"$dbuser\",\"$dbpass\",\"$ilines_spisok\",\"$page_id\")'
							        style='border:none'><img src='/calendar/images/calRight.gif' title='Следующий месяц' /></a>
							<a href='javascript:void(0)' onClick='showJump(this)' style='border:none'>
							    <img src='/calendar/images/calDown.gif' title='Выбрать месяц, год' /></a>
						</td>
						<td align='right'>
							<a title='Новости за месяц' href=$refmon>&nbsp;$name&nbsp;</a>
							<a title='Новости за год' href=$refyear>&nbsp;$year2&nbsp;</a>
						</td>
					</tr>
					</table>
				</td>
           ...
           if(($cur==$today[mday]) && ($nameen==$today[month]) && ($year2==$today[year]))
            ...
            if (strpos($sp,";".$cur.";")!=0 || ($cur==1 && substr($sp,0,3)==";1;")) {
				...
				if (strlen($mmm)>1)
					...
					$td=$year.".0".$mmm.".".$ddd;
			    $xml.="<div class='calevent'><b><a title='"."Посмотреть за новости за эту дату' href='/index.php?page_id=".$page_id."&td1=".$td."&td2=".$td."'>";
...
////
            ...
            if ($week_event)
            {
                ...
                $xml.="<div class='calevent'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a title='Посмотреть новости за неделю' href='/index.php?page_id=".$page_id."&td1=".$td1."&td2=".$td2."'><b>обзор</b></a></div>";
            }
            else
               $xml.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>обзор</b>&nbsp;&nbsp;&nbsp;&nbsp;";
	...
	$xml.="</table>";
    $xml.="<div ><strong>Сегодня: </strong>".date("d.m.Y")."</div>";
}
/*
 $diser0=mysql_query("SELECT c.icont_text AS date2 FROM adm_ilines_content AS c
                         INNER JOIN adm_ilines_content AS s ON s.el_id=c.el_id AND s.icont_var='status' AND s.icont_text='1'
                         INNER JOIN adm_ilines_element AS e ON e.el_id=c.el_id AND itype_id=14 ".
                         " WHERE c.icont_var='date2' AND c.icont_text > '".date('Y.m.d.')."'".
                         " ORDER BY  date2 ASC LIMIT 1");
    $diser=mysql_fetch_array($diser0);

     $xml.= "<div style='text-align:left;'><strong>Защита диссертаций: </strong>".substr($diser[date2],8,2).".".substr($diser[date2],5,2).".".substr($diser[date2],0,4)."</div>";
*/
