...
class QuestionnaireResultList implements PageBuilder {
...
//        ini_set('display_errors', 1);
        ...
        if($this->academicCouncilModule->getAuthorizationService()->isAuthorized()) {
            ...
            if(!empty($members)) {
                ...
                if($questionnaireId->isSecret()) {
                    $secretMembers = $this->academicCouncilModule->getQuestionnaireService()->getQuestionnaireSecretMembersListById($questionnaireId->getId());
                }
                ...
                if($questionnaireId->isSecret()) {
                    $section->addText(
                        mb_convert_encoding('Опросный лист участников предварительного тайного голосования', "UTF-8", "windows-1251"),
                        array('name' => 'Arial', 'size' => 12, "bold" => true),
                ...
                if($questionnaireId->isSecret()) {
                    $cell = $table->addCell(\PhpOffice\PhpWord\Shared\Converter::cmToTwip(12.75));
                    $cell->addText(
                        mb_convert_encoding('Код участника', "UTF-8", "windows-1251"),
                ...
                $counter=1;
                if($questionnaireId->isSecret()) {
                    foreach ($secretMembers as $secretMember) {
                        $table->addRow();
                        ...
                        $cell->addText(
                            mb_convert_encoding($secretMember->getCode(), "UTF-8", "windows-1251"),
                            array('name' => 'Arial', 'size' => 12),
                        ...
                        $cell = $table->addCell(\PhpOffice\PhpWord\Shared\Converter::cmToTwip(6));
                        $cell->addText(
                            mb_convert_encoding($secretMember->getVoteResult(), "UTF-8", "windows-1251"),
                            array('name' => 'Arial', 'size' => 12),
                            array('align' => 'center', 'spaceAfter' => 0,'spacingLineRule' => 'single','space' => array('line' => 240,'type' => 'single'))
                        );
                        $cell = $table->addCell(\PhpOffice\PhpWord\Shared\Converter::cmToTwip(7));

                        $notes = str_replace("\r\n","\n",$secretMember->getNotes());
                        $notesArr = explode("\n",$notes);
