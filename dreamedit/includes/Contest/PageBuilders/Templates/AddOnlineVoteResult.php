        ...
        if($currentUser->getStatus()->isAdmin() && !empty($_GET['contest_id']) && is_numeric($_GET['contest_id'])) {
            ...
            if(!empty($contest)) {
                if($contest->isOnlineVote()) {
                    ...
                    $formBuilder->registerField(new \FormField("second_place", "select", false, "Выбор 2-ого места", "", "", false, "", $secondPlace, $applicantSelectArr));
                    ...
                    foreach ($contest->getApplicants() as $applicant) {
                        ...
                        $formBuilder->registerField(new \FormField($applicant->getId() . "__online_vote_manual_total", "text", false, "Общее количество баллов", "", "", false, "", $total, array(), array(), "col-lg-12"));
