    ...
    public function build($params = array())
    {
        ...
        if($currentUser->getStatus()->isAdmin()) {
            ...
            if(!empty($_GET['delete_id'])) {
                $user = $this->contest->getUserService()->getUserById($_GET['delete_id']);
                if(!empty($user)) {
                    if($user->getStatus()->getId()==5) {
                        $this->contest->getUserService()->deleteUserById($_GET['delete_id']);
                    } else {
                        $this->contest->getUserService()->changeUserStatusByIdWithId($_GET['delete_id'],3);
                    }
                }
                exit;
            }
            ...
            if(!empty($_GET['recover_id'])) {
                $this->contest->getUserService()->changeUserStatusByIdWithId($_GET['recover_id'],2);
                exit;
            }
        }
        if($_GET['deleted']==1) {
                            ...
                            <div class="col">
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                        <?php
                                        $users = $this->contest->getUserService()->getAllUsers("id");
                                        foreach ($users as $user):
                                            ...
                                            ?>
                                            <tr>
                                                <th scope="row"><?= $user->getLastName() . " " . $user->getFirstName() . " " . $user->getThirdName() ?></th>
