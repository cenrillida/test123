...
<!--this is version v20110616, see http://www.monperrus.net/martin/bibtexbrowser/ -->
...
// Wednesday, June 01 2011: bug found by Carlos Brбs
...
define('BIBTEXBROWSER','v20110616');
...
@define('ENCODING','utf-8');//define('ENCODING','windows-1252');
// number of bib items per page
...
//  $_GET[Q_FILE]="/home/www/inion/html/bib3.bib";

  if (empty($file)) $file='bib3.bib';
  $uploaddir = $_CONFIG['global']['paths'][admin_path]."zotero/";
  $_POST[fname]= $uploaddir.$file;
...
//  $_GET[Q_FILE] = urldecode($_GET[Q_FILE]);
  ...
  /*
  foreach(explode(MULTIPLE_BIB_SEPARATOR, $_POST[fname]) as $bib) {
  ...
  */
  ...
  $compiledbib = 'bibtexbrowser_'.md5($_POST[fname]).'.dat';
...
//    $db->bibdb;
//  echo "<br />____f "; print_r($f);print_r($_GET[Q_DB]);
...
//function s3988($s) {return urlencode($s);}
/** @deprecated */
...
class BibEntry {
  ...
  function getURL() {
    ...
    else return "https://".$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME']).'/'.BIBTEXBROWSER_URL.'?'.createQueryString(array('key'=>$this->getKey()));
  }
  ...
  */
  ...
  function addHomepageLink($author) {
    ...
    // e.g. @STRING{hp_Jean-MarcJГ©zГ©quel="http://www.irisa.fr/prive/jezequel/"}
  ...
  */
////////////////
function toCoinsMySQL($publ,$author)
{
if ($_SESSION[lang]=='/en')
{
   $suff='_en';
...
$this->fields = array();
$this->setField('type','dc');
if ($publ[vid]==1)
	$this->setField('type','book');
else
{
if ($publ[vid]==2)
	$this->setField('type','journal');
}
//print_r($publ);
$this->setField('title',$publ['name'.$suff]);
$this->setField('atitle',$publ[name.$suff]);
$this->setField('jtitle',$publ[j_name.$suff]);
//echo "<br />".iconv("ISO-8859-1","windows-1251",$name);
//$this->setField('title',utf8_decode($name));
$this->setField('shorttitle',$publ[jtitle]);
$this->setField('isbn',str_replace("ISBN","",$publ[izdat]));
$this->setField('issn',$publ[issn]);
$this->setField('abstract',$publ[annots].$suff);
$this->setField('issue',$publ[number]);
if (empty($publ[number]))
    $this->setField('issue',$publ[issue]);
//$this->setField('volume',$publ[number]);
//$this->setField('journal',iconv("ISO-8859-1","windows-1251",$publ[jtitle]));
$this->setField('author',$author);
$this->setField('year',$publ[year]);
//$this->setField('subject','слово один, слово два,key');
//$this->setField('subject',$publ[keyword]);
//$this->setField('subject','слово один');
//$this->setField('subject','слово два');
$this->setField('subject',str_replace('<br>',',',$publ[keyword]));

$pp=explode("-",str_replace("--","-",$publ[pages]));

$this->setField('spage',$pp[0]);
$this->setField('epage',$pp[1]);
$this->setField('tpage',$publ[pages]);
$this->setField('journal',$publ[j_name]);
return $this->toCoins();
}
...
// echo "<br />".$publ[vid]; print_r($this);
    $url_parts=array();
    $url_parts[]='ctx_ver=Z39.88-2004';
// echo "<br />___gettitle ".$this->getTitle()."###";
    $type = $this->getType();
    ...
    if ($type=="book") {
      ...
      $url_parts[]='rft.isbn='.s3988($this->getField("isbn"));
      ...
      $url_parts[]='rft_val_fmt='.s3988('info:ofi/fmt:kev:mtx:dc');
      $url_parts[]='rft.title='.s3988($this->getTitle());
      $url_parts[]='rft.jtitle='.s3988($this->getField("jtitle"));
      $url_parts[]='rft.volume='.s3988($this->getField("volume"));
      $url_parts[]='rft.journal='.s3988($this->getField("jtitle"));
      $url_parts[]='rft.issue='.s3988($this->getField("issue"));

      $url_parts[]='rft.issn='.s3988($this->getField("issn"));
      $url_parts[]='rft.shorttitle='.($this->getField("shorttitle"));
      $url_parts[]='rft.subject='.s3988($this->getField("subject"));
      ...
      $url_parts[]='rft.title='.s3988($this->getTitle());
      $url_parts[]='rft.atitle='.s3988($this->getField("atitle"));
      $url_parts[]='rft.genre='.s3988($this->getField("article"));
      $url_parts[]='rft.jtitle='.s3988($this->getField("jtitle"));
      $url_parts[]='rft.volume='.s3988($this->getField("volume"));
      $url_parts[]='rft.journal='.s3988($this->getField("journal"));
      $url_parts[]='rft.abstract='.s3988($this->getField("abstract"));
      $url_parts[]='rft.issue='.s3988($this->getField("issue"));
      $url_parts[]='rft.spage='.s3988($this->getField("spage"));
      $url_parts[]='rft.epage='.s3988($this->getField("epage"));
      $url_parts[]='rft.tpage='.s3988($this->getField("tpage"));
      $url_parts[]='rft.number='.s3988($this->getField("number"));
      $url_parts[]='rft.year='.s3988($this->getField("year"));
      $url_parts[]='rft.issn='.s3988($this->getField("issn"));
      $url_parts[]='rft.shorttitle='.s3988($this->getField("shorttitle"));
      $url_parts[]='rft.subject='.s3988($this->getField("subject"));
// print_r($url_parts);
    }
...
?><?php /* pp4php: including bibtexbrowser/dev/bibtexbrowser-style-default.php (in bibtexbrowser/dev/bibtexbrowser.php:1403) */ ?><?php /** default bibliography btyle of bibtexbrowser
...
class BibtexBrowserDisplay {
  ...
  function poweredby() {
    ...
    $poweredby .= 'Powered by <a href="http://www.monperrus.net/martin/bibtexbrowser/">bibtexbrowser</a><!--v20110616-->';
    $poweredby .= '</div>'."\n";
    return $poweredby;
   }
...
class BibEntryDisplay extends BibtexBrowserDisplay {
  ...
  function metadata() {
    ...
    // this page
    $result[] = array('citation_abstract_html_url','https://'.$_SERVER['HTTP_HOST'].($_SERVER['SERVER_PORT']=='80'?'':$_SERVER['SERVER_PORT']).str_replace('&','&amp;',$_SERVER['REQUEST_URI']));
...
<meta name="generator" content="bibtexbrowser v20110616" />
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
   <channel>
      ...
      <link>https://<?php echo $_SERVER['HTTP_HOST'].htmlentities($_SERVER['REQUEST_URI']);?></link>
      <atom:link href="https://<?php echo $_SERVER['HTTP_HOST'].htmlentities($_SERVER['REQUEST_URI']);?>" rel="self" type="application/rss+xml" />
      <description></description>
      <generator>bibtexbrowser v20110616</generator>
...
//////////////////////
  ...
  function diagnosis() {
    ...
    echo "bibtexbrowser version: 20110616\n";
