...
( function() {
	...
	CKEDITOR.plugins.add( 'indenttext', {
		requires: 'indentt',
		icons: 'indentt,indentt-rtl,outdentt,outdentt-rtl', // %REMOVE_LINE_CORE%
		init: function( editor ) {
			var globalHelpers = CKEDITOR.plugins.indentt,
				classes = editor.config.indentClasses;

			// Register commands.
			globalHelpers.registerCommands( editor, {
				indenttext: new commandDefinition( editor, 'indenttext', true ),
				outdenttext: new commandDefinition( editor, 'outdenttext' )
			} );
			...
			function commandDefinition() {
				...
				this.allowedContent = {
					'div h1 h2 h3 h4 h5 h6 ol p pre ul': {
						...
						propertiesOnly: true,
						styles: !classes ? 'text-indent' : null,
				...
				this.requiredContent = ( this.enterBr ? 'div' : 'p' ) +
					( classes ? '(' + classes.join( ',' ) + ')' : '{text-indent}' );
	...
	function indentElement( element, classes, dir ) {
		...
		if ( classes ) {
			...
			var indentCssProperty = getIndentCss( element, dir ),
				...
				indentOffset = editor.config.indentOffset || 60;
	...
	function getIndentCss( element, dir ) {
		return ( dir || element.getComputedStyle( 'direction' ) ) == 'ltr' ? 'text-indent' : 'text-indent';
	}
