...
$cell_prefx = "a_";
$rows = $DB->select("SELECT * FROM ?_admin ORDER BY a_login DESC");
...
if(empty($rows))
{
	echo Dreamedit::translate("јдминистраторы отсутствуют");
	return;
}
...
foreach($rows as $row)
{
	...
	foreach($mod_tmp_vars as $tmpVar)
	{
		...
		if($tmpVar == "IMG")
		{
			if($row[$cell_prefx."status"] == 0)
				$mod_tmp_data[$tmpVar] = $ico["dis"];
			if($row[$cell_prefx."status"] == 1)
