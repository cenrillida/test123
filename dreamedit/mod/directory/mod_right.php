...
if($_ACTIVE["action"] == "edit")
{
	...
	{
		$lid = (int)@$_REQUEST["id"];
		$el = $directories->getElementById($id);
		$id = $el["itype_id"];

		getStructure($id);
		$phorm->add_comp("lid", array("class" => "base::hidden", "value" => $lid));
		$el = $directories->appendContent(array($lid => $el));
		$data = array();
	...
	{
		$row = $directories->getTypeById($id);
		$data = array();
		foreach($phorm->data() as $k => $v)
		{
			if(!isset($v["field"]))
...
if($_ACTIVE["action"] == "del")
{
	if($type == "l")
	{
		...
		$DB->query("DELETE FROM ?_directories_content WHERE el_id = ?d", $_GET["id"]);
		$DB->query("DELETE FROM ?_directories_element WHERE el_id = ?d", $_GET["id"]);
	...
	else
	{
		...
		$el_ids = $directories->getElementsByType(array($id));

		if(!empty($el_ids))
			$DB->query("DELETE FROM ?_directories_content WHERE el_id IN (?a)", array_keys($el_ids));
		$DB->query("DELETE FROM ?_directories_element WHERE itype_id = ?d", $id);
		$DB->query("DELETE FROM ?_directories_type WHERE itype_id = ?d", $id);
