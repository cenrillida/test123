...
function page_parents()
{
	...
	$rows = $DB->select("SELECT page_id, page_name, page_parent FROM ?_helpers ORDER BY page_parent ASC, page_position ASC");
	foreach($rows as $row)
	{
...
function site_templates()
{
	...
	foreach(glob($_SERVER["DOCUMENT_ROOT"]."/".$_CONFIG["global"]["paths"]["templates"]."varsh.*.php") as $tpl)
	{
		include $tpl;
