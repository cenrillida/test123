...
{
	global $DB, $phorm, $ilines;

	$phorm->remove_all();
	$itype_struct = $ilines->getTypeById($tid);

	$structure = xml_get_array_from_xml_string($itype_struct["itype_auto_structure"]);
...
function convertIfNeed($text, $to) {
    if($to!="windows-1251") {
        return mb_convert_encoding($text,"windows-1251",$to);
    } else {
        return $text;
    }
}
