...
if($_ACTIVE["action"] == "del")
{
	if($type == "l")
	{
		...
		$DB->query("DELETE FROM ?_nirs_content WHERE el_id = ?d", $_GET["id"]);
		$DB->query("DELETE FROM ?_nirs_element WHERE el_id = ?d", $_GET["id"]);
	...
	else
	{
		...
		if(!empty($el_ids))
			$DB->query("DELETE FROM ?_nirs_content WHERE el_id IN (?a)", array_keys($el_ids));
		$DB->query("DELETE FROM ?_nirs_element WHERE itype_id = ?d", $id);
		$DB->query("DELETE FROM ?_nirs_type WHERE itype_id = ?d", $id);
