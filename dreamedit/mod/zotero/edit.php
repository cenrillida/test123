...
function ZTCheck(avt)
{
    ...
    if(document.publ.vid.selectedIndex==0) {
		alert("Не выбран вид публикации");
		return false;
	}
...
//	alert ("*"+document.publ.annots.value+"*");
...
echo "<b>EDIT</b> ";
echo $_REQUEST["file"];
$a=setDB($_REQUEST["file"]);
$bib0=$a->bibdb;
//print_r($bib0);
$bow=$bib0[$_REQUEST[id]];
echo "id=".convutf8($bow->fields[title])."<br />";
//print_r($bow);
//echo convutf8($bow->fields[annote]);
$avt00=convutf8($bow->fields[author]);
$avt0=explode(" and ",$avt00);
?>

<form name=publ enctype="multipart/form-data" action=index.php?mod=zotero&action=save method=post  onSubmit="return ZTCheck(<?=@count($avt0)?>)">
<input type=hidden name=sent value=1>
<font color=red>*</font>Название (полная библиографическая ссылка)
<br>
<textarea name=name cols=120><? echo convutf8($bow->fields[title]); ?></textarea>
<br>
Название на английском
<br>
<textarea name=name2 cols=120><? echo convutf8($bow->fields[title]); ?></textarea>
<br>
<table>
<tr>
<td>
...
<br>
<input type=text name=date maxlength=4 value='<? echo $bow->fields[year]; ?>'  style="width: 50">
</td>
<td>
...
<br>
<textarea name=izdat cols=120 rows=1><? echo $bow->fields[isbn]; ?></textarea>
<br>
...
<br>
<input name=pages value='<? echo $bow->fields[number]; ?>'></input>

<br><br />
<b>Номера cтраниц</b>

<br>
<input name=pages size=80 value='<? echo $bow->fields[pages]; ?>'></input>
...
foreach($avt0 as $avt)
{
   echo "<tr><td><div id='ffioname".$iavt."'<b>".trim($avt)."</b></div></td>";
   echo "<td>&nbsp;&nbsp;&nbsp;</td>";
   ...
   {
   	   if(trim($aa[0]) == trim($a[surname])) $sel=' selected '; else $sel='';
