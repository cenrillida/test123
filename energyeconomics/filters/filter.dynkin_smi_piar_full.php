...
$query = "SELECT c.el_id AS id,c.icont_text AS prev_text,rub.icont_text AS rubric,a.icont_text AS title,pe.icont_text AS people,sp.icont_text AS small_picture,d.icont_text AS date,f.icont_text AS full_text,
                 ...
                 INNER JOIN adm_ilines_content AS dyn ON dyn.el_id=a.el_id AND dyn.icont_var='dynkin_smi'
				 ...
				 WHERE a.icont_var='title' AND s.icont_text=1 AND dyn.icont_text=1
...
if (isset($_GET['page_dynkin_smi'])) $page=($_GET['page_dynkin_smi']-1); else $page=0;
...
$query = "SELECT c.el_id AS id,c.icont_text AS prev_text,rub.icont_text AS rubric,a.icont_text AS title,pe.icont_text AS people,sp.icont_text AS small_picture,d.icont_text AS date,f.icont_text AS full_text,
				 ...
				 INNER JOIN adm_ilines_element AS e ON e.el_id=a.el_id AND e.itype_id=5
				 WHERE a.icont_var='title' AND s.icont_text=1 AND dyn.icont_text=1
                 ORDER BY d.icont_text DESC LIMIT ".$start.",".$mat_per_page or die("Error in the consult.." . mysqli_error($link)); 
$result = $link->query($query); 
...
echo '<br><b>Страницы:</b>';
  ...
  if($page!=0)
  	echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page_dynkin_smi='.($page).'">'."<b> предыдущая&nbsp;&nbsp;←</b>"."</a> ";
...
for($i=$page+1;$i<=$total_pages;$i++) {
  if ($i-1 == $page) {
    ...
    echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page_dynkin_smi='.$i.'">'.$i."</a> ";
  }
  ...
  if($total_pages>$page+1)
  	echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page_dynkin_smi='.($page+2).'">'."<b>→&nbsp;следующая</b>"."</a> ";
...
echo '<table bgcolor="#f7f7f7" cellpadding="6" width="100%">
	<tbody><tr>
		<td style="padding: 10px 5px 10px 10px;" valign="top">
			'.$img_smi_piar.'	</td>
