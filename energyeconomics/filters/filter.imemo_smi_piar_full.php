...
$query = "SELECT c.el_id AS id,c.icont_text AS prev_text,rub.icont_text AS rubric,a.icont_text AS title,pe.icont_text AS people,sp.icont_text AS small_picture,d.icont_text AS date,f.icont_text AS full_text,
				 ...
				 WHERE a.icont_var='title' AND s.icont_text=1 AND rub.icont_text=439
...
if (isset($_GET['page_smi'])) $page=($_GET['page_smi']-1); else $page=0;
...
echo '<br><b>Страницы:</b>';
  ...
  if($page!=0)
  	echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page_smi='.($page).'">'."<b> предыдущая&nbsp;&nbsp;←</b>"."</a> ";
...
for($i=$page+1;$i<=$total_pages;$i++) {
  if ($i-1 == $page) {
    ...
    echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page_smi='.$i.'">'.$i."</a> ";
  }
  ...
  if($total_pages>$page+1)
  	echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page_smi='.($page+2).'">'."<b>→&nbsp;следующая</b>"."</a> ";
...
if($rows[people]!='' && $rows[people]!='<br>')
{
	...
	$img_smi_piar="<p><img width=90 height=120 src=../dreamedit/foto/".$row_picbig_smi_piar[picbig]."></img></p>";
}
