...
<script>
    ...
    if (($handle = fopen("/home/imemon/html/energyeconomics/tables/china_data.csv", "r")) !== FALSE) {
        ...
        while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
            ...
            if(!empty($contries[$row-$minus]))
            {
                for ($i=0; $i <= 5; $i++) { 
                ?>
                    oil_request['<?=$current_scenario?>'][0]['<?=$contries[$row-$minus]?>']['<?=$years_arr[$i]?>'] = '<?=$data[1+$i]?>';
                    oil_request['<?=$current_scenario?>'][1]['<?=$contries[$row-$minus]?>']['<?=$years_arr[$i]?>'] = '<?=$data[8+$i]?>';
                    oil_request['<?=$current_scenario?>'][2]['<?=$contries[$row-$minus]?>']['<?=$years_arr[$i]?>'] = '<?=$data[15+$i]?>';
                    oil_request['<?=$current_scenario?>'][3]['<?=$contries[$row-$minus]?>']['<?=$years_arr[$i]?>'] = '<?=$data[22+$i]?>';
                <?
                }
            }  
            if($row==20) {
    ...
    function base_sc() {
        ...
        <?php
$result_maxing = $link->query("SELECT * FROM cer_model3_settings LIMIT 1");
