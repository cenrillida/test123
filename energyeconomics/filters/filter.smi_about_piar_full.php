...
echo '<br><b>Страницы:</b>';
  ...
  if($page!=0)
  	echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page='.($page).'">'."<b> предыдущая&nbsp;&nbsp;←</b>"."</a> ";
...
for($i=$page+1;$i<=$total_pages;$i++) {
  if ($i-1 == $page) {
    ...
    echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page='.$i.'">'.$i."</a> ";
  }
  ...
  if($total_pages>$page+1)
  	echo '<a href="'.$_SERVER['PHP_SELF'].'?page_id='.$_GET['page_id'].'&page='.($page+2).'">'."<b>→&nbsp;следующая</b>"."</a> ";
