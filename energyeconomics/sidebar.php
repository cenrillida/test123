...
$query = "SELECT ac.el_id,ac.icont_text FROM adm_headers_element AS ae INNER JOIN adm_headers_content AS ac ON ac.el_id=ae.el_id WHERE ae.itype_id=".$sidebar_id." AND ac.icont_var='sort' GROUP BY el_id ORDER BY ac.icont_text" or die("Error in the consult.." . mysqli_error($link)); 
...
if($color_fp_title=="white")
{
	...
	echo'<a href=index.php?page_id='.$arr_fp['link_id'][$j_f].'><h6>'.$arr_fp['title'][$j_f].'</h6></a>';
	else
	echo'<a target="_blank" href=../index.php?page_id='.$arr_fp['link_id'][$j_f].'><h6>'.$arr_fp['title'][$j_f].'</h6></a>';
	}
	else
	echo'<h6>'.$arr_fp['title'][$j_f].'</h6>';
	echo '</div>';
}
else
{
