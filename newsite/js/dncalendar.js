 ...
 (function ( $ ) {
      ...
      return weeks;
		...
		*/
		var draw = function() {
          ...
          var prevLinkGroup = $("<div id='dncalendar-prev-month' class='dncalendar-prev-month'><i class='fas fa-2x fa-arrow-left'></i></div><div id='dncalendar-prev-year' class='dncalendar-prev-year'><i class='fas fa-2x fa-angle-double-left'></i></div>");
          var nextLinkGroup = $("<div id='dncalendar-next-month' class='dncalendar-next-month'><i class='fas fa-2x fa-arrow-right'></i></div><div id='dncalendar-next-year' class='dncalendar-next-year'><i class='fas fa-2x fa-angle-double-right'></i></div>");
          ...
          if (weekNamesHasAlreadySorted == false) {
                ...
                for (var i = 0; i < weekName.length; i++) {
                      if (i >= dayIndex) {
                            ...
                            var old_index = oldIndex;
                            var new_index = newIndex;

                          while (old_index < 0) {
                              old_index += weekName.length;
                          }
                          while (new_index < 0) {
                              new_index += weekName.length;
                          }
                          if (new_index >= weekName.length) {
                              var k = new_index - weekName.length;
                              while ((k--) + 1) {
                                  weekName.push(undefined);
                              }
                          }
                          weekName.splice(new_index, 0, weekName.splice(old_index, 1)[0]);
    ...
    var getNotesThisMonth = function() {
		...
		var nextMonth = function() {
            ...
            var jqxhr = $.ajax( "/dreamedit/filters/dncalendar.php?month=" + (currDate.getMonth() + 1) + "&year="  + currDate.getFullYear() + "&lang=" + lang)
                .done(function(result) {
                    var notesDay = [];
                    result.forEach(function(entry) {
                        notesDay.push({"date": currDate.getFullYear() + '-' + (currDate.getMonth() + 1) + "-" + entry.date_event, "note": ["Пометка"]});
                    });
                    my_calendar.update({
                        notes: notesDay
                    });
    ...
    var prevYear = function() {
			var firstDateOfPrevYear = new Date(currDate.getFullYear() - 1, currDate.getMonth(), 1); // get fist day of previous year
			var date = firstDateOfPrevYear.getDate();
			var month = firstDateOfPrevYear.getMonth();
			var year = firstDateOfPrevYear.getFullYear();
    ...
    var nextYear = function() {
      var firstDateOfNextYear = new Date(currDate.getFullYear() + 1, currDate.getMonth(), 1); // get fist day of previous year
      var date = firstDateOfNextYear.getDate();
      var month = firstDateOfNextYear.getMonth();
      var year = firstDateOfNextYear.getFullYear();
      ...
      draw();
		...
		var triggerAction = function() {
		    ...
		    $('.main-menu-block-element-calendar').on('click', '#calendarClick', function(){
		    ...
		    $('.main-menu-block-element-calendar').on('click', '#dncalendar-prev-month', function() {
		    	prevMonth();
		    });

		    $('.main-menu-block-element-calendar').on('click', '#dncalendar-next-month', function() {
		    	nextMonth();
		    });

        $('.main-menu-block-element-calendar').on('click', '#dncalendar-prev-year', function() {
		    	prevYear();
		    });

        $('.main-menu-block-element-calendar').on('click', '#dncalendar-next-year', function() {
		    	nextYear();
		    });
		}
