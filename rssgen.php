...
$rss_logo="https://".$rss_site."/img/logo_print.jpg";
...
for($i=0;$i<10; $i++) //will loop through 6 times to generate 6 RSS items
{
	...
	$photo_name = "https://".$rss_site."/files/Image/logo_imemo.jpg"; //'where-ever a photo for this article could be found - needs to be http://www.yoursite.com/images/whatever.gif';
...
//	$url_product = '/articles/wherever_your_article/blog_can_be_found'; //define the URL of where people could read this blog/article entry
    $url_product="/";
    if ($subjectn ==1) $url_product = '/index.php?page_id=502&amp;id='.mysql_result($result,$i,'el_id').'';
    if ($subjectn ==4) $url_product = '/index.php?page_id=503&amp;id='.mysql_result($result,$i,'el_id').'';
	if ($subjectn ==5) $url_product = '/index.php?page_id=502&amp;id='.mysql_result($result,$i,'el_id').'';
	if ($subjectn ==3) $url_product = '/index.php?page_id=502&amp;id='.mysql_result($result,$i,'el_id').'';
	...
	$pubdate = substr($dd,8,2).".".substr($dd,5,2).".".substr($dd,0,4);
    ...
    echo			'</title>
				<link>https://'.$rss_site.$url_product.'</link>
				<guid isPermaLink="true">https://'.$rss_site.$url_product.'</guid>
					<description>'.$short_description.'</description>
					<pubDate>'.$subjectd.'</pubDate>
		</item>
