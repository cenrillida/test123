...
if($_SESSION["lang"] != "/en")
{
	$lang_suf = "_RU";
	$txt_rub='Рубрика';
  $txt_type='Тип публикации';
  $relatedText = "Еще публикации по теме";
}
else
{
	$lang_suf = "_EN";
...
$result=$DB->select(
	 ...
	 IF(publ.vid_inion>0,
	 CONCAT('<a href=/index.php?page_id=',".$_TPL_REPLACMENT[FULL_ID].",'&vid=',vi.el_id,' title=\"Все публикации этого вида\" >',vi.icont_text,'</a>'),'') AS vid_inion, vi.icont_text AS vid_text, vi.el_id AS vid_id
	          FROM publ
...
<br />
<? 
if ($_SESSION[lang]!='/en' && !empty($row[rinc])) echo "<a href=".$row[rinc]." target=_blank>Размещено в РИНЦ </a><br /><br />";
...
if ($_SESSION[lang]!='/en') {
    if($row['vid_id']!=428 || $row['year']!=2020) {
        echo "<div style=\"background-color: #EEE9E9; color: grey; padding: 0 10px;\"><br />Ссылка при цитировании: <br /><br />";
