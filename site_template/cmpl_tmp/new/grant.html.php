...
$tt="tpl.".$tplname.".html";
 ...
 echo "<strong>Список по годам:&nbsp;&nbsp;&nbsp; </strong>";
 for($i=$y1;$i>=$years['beg'];$i=$i-1)
 {
    $ii="<a href=/index.php?page_id=".$_REQUEST[page_id]."&year=".$i." title='Список грантов за ".$i." год'>";
    if ($i==$y0)
 ...
 echo "<br /><br />";
 echo "<font size='3'><strong>".$y0." год</strong></font><br /><br />";
...
// echo $v[content]["TITLE"];
		...
		if(isset($rows[@$_REQUEST["id"]]["content"]["DATE"]))
	{
		...
		if(!empty($v['fioexe'])) {
            $tpl->appendValues(array("fioexe" => "<a href=/index.php?page_id=" . $_TPL_REPLACMENT[PERS_ID] . "&id=" . $v[idpersonsexe] . " title='Информация об исполнителе'>" . $v["fioexe"] . "</a>"));
        }
