...
if ($_SESSION[lang]!='/en')
$rows=$DB->select("SELECT page_name AS name FROM adm_magazine WHERE  page_id=".$_REQUEST[id]);
else
$rows=$DB->select("SELECT page_name_en AS name FROM adm_magazine WHERE  page_id=".$_REQUEST[id]);
	...
	for ($iy=$yearmax;$iy>=$yearmin;$iy=$iy-1)
		 {
			...
			{
			    if (!empty($mag[$iy][$in]))
					echo "<td align=center><a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARCHIVE_ID]."&jid=".$mag[$iy][$in].">".$in."</a></td>";
				else
...
echo "<a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[FULL_ID].">".$txt."</a>";
