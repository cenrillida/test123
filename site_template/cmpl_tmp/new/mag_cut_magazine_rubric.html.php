...
else
{
   ...
   $rows=$DB->select("SELECT page_id,IFNULL(name".$suff.",page_name) AS rubric,journal FROM adm_article
					  ...
					  AND IFNULL(name".$suff.",page_name) LIKE '".str_replace("_"," ",$_REQUEST[rub])."'");
 //print_r($rows);
   $whererub="";
   foreach($rows as $row)
   {
...
$site_templater->displayResultFromPath($_CONFIG["global"]["paths"]["template_path"]."top.text.html");
 echo "<br />"; 
  if ($_SESSION["lang"]!='/en')
  {
	echo "<a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT['RUBRICLIST_ID'].">К списку рубрик</a><br />";
	// echo "<div class='jrubric'>Статьи в рубрике: <br />".$rows[0][rubric]."</div><br />";
  }
  else	
  {
	echo "<a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT['RUBRICLIST_ID'].">Rubrics List</a><br />";
...
$rows=$DB->select("SELECT a.page_id,a.name".$suff.",a.people,a.year,a.number,a.page_template 
				  ...
				  " AND r.page_template='jrubric' AND r.page_status=1 ".
				   ...
				   AND  a.page_status=1 
				   ORDER BY year DESC,number DESC,a.page_position,a.name".$suff);
  	  ...
  	  switch($row[page_template])
      {
      	...
      	case "jrubric":
		   ...
		   if (!($row[page_id]==$pageid_jour))
	      	   ...
	      	   echo "<div class='jrubric2'><a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[RUBRIC_ID]."&id=".$row[page_id]."&jid=".$_REQUEST[jid]."&jj=".$_REQUEST[jj].">".$row[page_name]."</a></div>";
      	...
      	case "jarticle" :
		   ...
		   $avt22='';
      	   foreach($people0 as $people)
      	   {
			  ...
			  $avt22.=" <a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[PERSONA_ID]."&id=".$people[id].
		   ...
		   echo $avt22;
      	   echo "</div>";
      	   echo "<div class='name'><a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARTICLE_ID]."&id=".$row[page_id]."&jid=".$_REQUEST[jid]."&jj=".$_REQUEST[jj].">".$row['name'.$suff]."</a></div>";
      	   echo "</div>";
           break;
      }
	}
