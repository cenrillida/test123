...
<?
 ...
 if (!$_TPL_REPLACMENT[SHORT_LIST]==1)
 {
  ...
  foreach($rows as $row)
  {
     echo "<a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$pageid."&year=".$row[year].">".$row[year]."</a> | ";
  ...
  if ($_TPL_REPLACMENT[SHORT_LIST]==1)  //Проверить можно ли выдать краткий список
  {
	...
	$rows=$DB->select("SELECT IFNULL(r.name,r.page_name) AS name,
	 ...
	 foreach($rows as $row)
	 {
			...
			if ($_SESSION[lang]!='/en')
			{
			echo "<li><a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[RUBRIC_ID]."&rub=".
  ...
  if (!empty($row[content][RUBRIC]))
     {
	        ...
	        if ($rubric != $row[content][RUBRIC])
	        {
				...
				echo "<div class='".$class."'>".$sim."<a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[RUBRIC_ID].
...
//echo $_SESSION[lang];
	echo "<div class='name_r'><b><a href=".$_SESSION['lang']."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[CONTENT_ID].
			"&jid=".$row[page_parent].">".
			$ntxt.$row[number]."-".$row[year]."</a></b></div>";
//	echo $_SESSION[lang];		
		...
		if (!empty($people[id]))
			      	      ...
			      	      if(substr($people[fio],0,8)!='редакция' && substr($people[fio],0,8)!='Редакция') //907
      	    			  {
				      	      if ($people[otdel]!='Умершие сотрудники')
      	        				  $avtList.="<a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[PERSONA_ID]."&id=".$people[id].
...
//	    			echo "<div class='jarticle' style='padding-left:60px;'>".$art[name]."</div>";
                    if ($_SESSION["lang"]!='/en' || empty($art[name_en]))
 			     	 echo "<div class='name'><a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARTICLE_ID]."&id=".$art[page_id]."&jid=".$_REQUEST[jid]."&jj=".$_REQUEST[jj].">".$art[name]."</a></div>";
                    else 
      		     	 echo "<div class='name'><a href=".$_SESSION["lang"]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARTICLE_ID]."&id=".$art[page_id]."&jid=".$_REQUEST[jid]."&jj=".$_REQUEST[jj].">".$art[name_en]."</a></div>";
	    		    ...
	    		    foreach($art20 as $art2)
    				{
		      	   ...
		      	   foreach($people0 as $people)
      			   {
      	      		if (!empty($people[id]))
      	    			  ...
      	    			  {
				      	      if ($people[otdel]!='Умершие сотрудники')
      	        				  $avtList.="<a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[PERSONA_ID]."&id=".$people[id]."&at=a>".$people[fio]."</a>, ";
		      			      else
                     	           $avtList.="<a style='border:1px solid gray;' href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id".$_TPL_REPLACMENT[PERSONA_ID]."&id=".$people[id]."&at=a>".$people[fio]."</a>, "; //.$people[work].",".$people[mail1]."";
                          }
                     }
                   }
          		   echo "<div class='autors'>";
...
//	    			echo "<div class='jarticle' style='padding-left:60px;'>".$art[name]."</div>";
 			     	 echo "<div class='name'><a href=".$_SESSION[lang]."/jour_cut/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARTICLE_ID]."&id=".$art2[page_id]."&jid=".$art2[jid]."&jj=".$_REQUEST[jj].">".$art2[name]."</a></div>";
