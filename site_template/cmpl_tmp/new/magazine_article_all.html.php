...
if ($_SESSION[lang]!='/en')
{
 for ($i = ord("А"); $i <= ord("Я"); $i++)
 {
		...
		$href="<a style='text-decoration: none;' href=/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_REQUEST[page_id]."&alf=".chr($i).">";
	    else $href="<a href='#". $i."'>";	
 	    if ((chr($i) != 'Ъ') && (chr($i) != 'Ь') && (chr($i) != 'Ы'))
		echo $href."<b>".chr($i)." </b></a>&nbsp";
  }  
  echo "<a style='text-decoration: none;' href=/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_REQUEST[page_id]."&alf=z><b>A...Z</b></a><br><br>";
}  
else
{
...
if ($_TPL_REPLACMENT[	SHORT_LIST]!=1) 
    echo "<br /><a title='Информация о статье' href=".$_SESSION[lang]."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARTICLE_ID]."&id=".$row[page_id].">".
	 $row['name']."</a><br />"; 	  
...
//  print_r($avt);
  ...
  foreach($people0 as $a)
  {
	 ...
	 echo "<a title='Информация об авторе' href=".$_SESSION[lang]."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[PERSONA_ID]."&id=".$a[id].">".$a[fio].'</a> ';
  }
 ...
 if (!empty($row[page_name]))
  echo "<br /><a title='Содержание выпуска' href=".$_SESSION[lang]."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[CONTENT_ID].
	 "&jid=".$row[jid].">".
