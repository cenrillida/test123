...
if(!$title_flag) {
    ...
    if (!empty($_REQUEST[jj])) {
        $rows = $DB->select("SELECT page_name,page_name_en FROM adm_magazine WHERE page_id=" . (int)$_REQUEST[jj]);
        if ($_SESSION[lang] != '/en')
...
if ($_SESSION[lang]!='/en')
{
 for ($i = ord("А"); $i <= ord("Я"); $i++)
         ...
         echo "<a style='text-decoration: none;' href="."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_REQUEST[page_id]."&alf=".chr($i)."><b>".chr($i)." </b></a>&nbsp";
         echo "<a style='text-decoration: none;' href="."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_REQUEST[page_id]."&alf=z><b>А...Я</b></a><br><br>";
}  
else
{
for ($i = ord("A"); $i <= ord("Z"); $i++)
 	    
         echo "<a style='text-decoration: none;' href=/en/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_REQUEST[page_id]."&alf=".chr($i)."><b>".chr($i)." </b></a>&nbsp";
...
foreach($a as $nid=>$nn)
{
   ...
   echo "<td valign='top' width=250px>";
    ...
    echo "<a class='authors_list' style='border:none;' href=".$_SESSION["lang"]."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[PERSONA_ID]."&id=".$nn[avtor_id].
	...
	foreach($idd as $a)
   {
	...
	{
		if ($a!='')
		echo "<a href=".$_SESSION["lang"]."/jour/".$_SESSION[jour_url]."/index.php?page_id=".$_TPL_REPLACMENT[ARTICLE_ID]."&id=".$ida[$k].">".str_replace("Запад - Во-", "Запад - Восток - Россия ", $a)."</a> | ";
