		...
		<script type="text/javascript">
			function change_table() {
		        ...
		        if(jQuery( "#clue_words_select option:selected").text()!="Все") {
		                jQuery('.oni-documents-table').find('.clue-td').each(function(index){ if(jQuery(this).html().toLowerCase().indexOf(jQuery( "#clue_words_select option:selected").text().toLowerCase()) < 0) jQuery(this).closest('table').closest('tr').hide();});
		            }
		         if(jQuery('#word_input').val()!="") {
		         	jQuery('.oni-documents-table').find('.clue-td').each(function(index){ if (jQuery(this).html().toLowerCase().indexOf(jQuery('#word_input').val().toLowerCase()) < 0 && jQuery(this).parent().find(".anot-text-td").html().toLowerCase().indexOf(jQuery('#word_input').val().toLowerCase()) < 0) jQuery(this).closest('table').closest('tr').hide();});
				...
				foreach ($test_xml->Worksheet[0]->Table->Row as $row) {
					...
					if(!empty($row->Cell[0]->Data[0])) {
						...
						foreach ($row->Cell as $cell) {
							...
							if($cell_number>=11 && $cell_number<15) {
								...
								case 17:
									...
									<td class="anot-td">
										<div class="buts"><a class="buts1" onclick="return false;" href="#">Подробнее</a>
										    <div class="buts_text" style="display: none;">
										    	<table class="oni-documents-table">
										    		<tr>
										    			<td>
