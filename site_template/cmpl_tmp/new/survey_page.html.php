...
global $_CONFIG, $site_templater, $DB;
	...
	if(!empty($_GET[p]))
	{
		$start=$count_survey*(int)$_GET[p]-$count_survey;
	}
	else
		$start=0;

		$rowsNoLimit=$DB->select("SELECT ae.el_id AS ARRAY_KEYS, question.icont_text AS question, answer1.icont_text AS answer1, answer2.icont_text AS answer2, answer3.icont_text AS answer3, answer4.icont_text AS answer4, answer5.icont_text AS answer5 FROM adm_ilines_element AS ae 
							...
							WHERE ae.itype_id=14 AND question.icont_text<>'' AND question.icont_text<>'<p>&nbsp;</p>' AND (off.icont_text=1) AND STR_TO_DATE(date_from.icont_text, '%Y.%m.%d %H:%i')<NOW() ORDER BY STR_TO_DATE(date_from.icont_text, '%Y.%m.%d %H:%i') DESC");
		
		if(count($rowsNoLimit)>$count_survey)
		{
			if(empty($_GET[p]))
			{
				$current_page=1;
			...
			$count_pages=ceil(count($rowsNoLimit)/$count_survey);
			for ($i=1; $i <=$count_pages ; $i++) { 
				if($i==1) {
					if($current_page!=1) {
						echo '<a href="'.$_SESSION[lang].'/index.php?page_id='.$_GET[page_id].'&p='.($current_page-1).'"><b>'.$prev_text.'&nbsp;&nbsp;</b></a>&nbsp;&nbsp; ';
					}
				}

					if($i!=$current_page)
						echo '<a style="text-decoration:underline;" href="'.$_SESSION[lang].'/index.php?page_id='.$_GET[page_id].'&p='.$i.'">'.$i.'</a>&nbsp;&nbsp; ';
					else
			...
			if($current_page!=$count_pages)
				echo '<a href="'.$_SESSION[lang].'/index.php?page_id='.$_GET[page_id].'&p='.($current_page+1).'"><b>&nbsp;&nbsp;&nbsp;   '.$next_text.'</b></a>';
		}
		...
		$rows=$DB->select("SELECT ae.el_id AS ARRAY_KEYS, question.icont_text AS question, answer1.icont_text AS answer1, answer2.icont_text AS answer2, answer3.icont_text AS answer3, answer4.icont_text AS answer4, answer5.icont_text AS answer5 FROM adm_ilines_element AS ae 
							...
							INNER JOIN adm_ilines_content AS date_to ON ae.el_id=date_to.el_id AND date_to.icont_var='date2'
							INNER JOIN adm_ilines_content AS off ON ae.el_id=off.el_id AND off.icont_var='status".$lang_suff."'
							WHERE ae.itype_id=14 AND question.icont_text<>'' AND question.icont_text<>'<p>&nbsp;</p>' AND (off.icont_text=1) AND STR_TO_DATE(date_from.icont_text, '%Y.%m.%d %H:%i')<NOW() ORDER BY STR_TO_DATE(date_from.icont_text, '%Y.%m.%d %H:%i') DESC LIMIT ".$start.",".$count_survey);
		//var_dump($rows);
